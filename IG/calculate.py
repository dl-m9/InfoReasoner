...
def check_entail(text1, text2, model, entailment_option='bi', threshold=DEFAULT_ENTAILMENT_THRESHOLD):
    # text1, text2: b, n
    ...
    entail_prob_1 = F.softmax(logits_1, dim=-1)[..., 2]
    entail_prob_2 = F.softmax(logits_2, dim=-1)[..., 2]

    if entailment_option == 'bi':
        semantically_equivalent = (entail_prob_1 > threshold) & (entail_prob_2 > threshold)
    elif entailment_option == 'a_entails_b': # text 1 entails text 2
        semantically_equivalent = (entail_prob_1 > threshold)
    elif entailment_option == 'b_entails_a': # text 2 entails text 1
        semantically_equivalent = (entail_prob_2 > threshold)
    ...
    entail_logits = {
        ...
        'a_entails_b_prob': entail_prob_1.detach(),
        'b_entails_a_prob': entail_prob_2.detach(),
    }
    return semantically_equivalent, entail_logits


def _connected_component_ids(num_items, pairwise_equivalence):
    adjacency = [[] for _ in range(num_items)]
    cursor = 0
    for i in range(num_items):
        for j in range(i + 1, num_items):
            if bool(pairwise_equivalence[cursor]):
                adjacency[i].append(j)
                adjacency[j].append(i)
            cursor += 1

    semantic_set_ids = [-1] * num_items
    next_id = 0
    for start in range(num_items):
        if semantic_set_ids[start] != -1:
            continue
        stack = [start]
        semantic_set_ids[start] = next_id
        while stack:
            current = stack.pop()
            for neighbor in adjacency[current]:
                if semantic_set_ids[neighbor] == -1:
                    semantic_set_ids[neighbor] = next_id
                    stack.append(neighbor)
        next_id += 1

    return semantic_set_ids, next_id
...
def calculate_uncertainty_soft_batch(
    ...
    """Compute soft-version IG for a batch of examples.

    When ``return_entailment`` is False (default), this matches the original
    behavior and returns a list of IG scores (one per example).

    When ``return_entailment`` is True, it additionally returns per-example
    entailment matrices that can be used to recompute IG/InfoGain offline
    for arbitrary subsets of responses.
    """
    bsz = len(example['question'])
    n_responses = len(example["response_text"][0])

    batch_questions = example['question']  # b
    batch_response_texts = example["response_text"]  # b, R
    batch_gt_answers = example['answers']  # b, A?
    likelihood = example['likelihood']  # b, R
    ...
    for i in range(0, len(all_responses), computation_chunk_size):
        ...
        # Note: GPU tensors will be cleaned up at worker level after all computations

    # response entails answer: 满分
    # answer entails response: 最多算一半
    # TODO: check whether this setting is optimal
    ig_scores = []
    entailment_details = [] if return_entailment else None
    for bi in range(bsz):
        reponse_entails_answer = np.array(
            all_reponse_entails_answer[bid_counter[bi]: bid_counter[bi + 1]]
        ).reshape(n_responses, -1)
        answer_entails_response = np.array(
            all_answer_entails_response[bid_counter[bi]: bid_counter[bi + 1]]
        ).reshape(n_responses, -1)
        '''
        # setting1
        entail_mass = np.stack(
            [
                likelihood[bi][..., np.newaxis] * reponse_entails_answer,
                0.5 * likelihood[bi][..., np.newaxis] * (reponse_entails_answer + answer_entails_response),
            ],
            axis=-1,
        )  # R, A, 2
        ...
        entail_mass = np.stack(
            [
                likelihood[bi][..., np.newaxis] * reponse_entails_answer,
                likelihood[bi][..., np.newaxis] * answer_entails_response,
            ],
            axis=-1,
        )  # R, A, 2
        try:
        ...
        if return_entailment:
            # Store per-example entailment matrices for offline recomputation.
            # We flatten matrices in row-major order to make them JSON-serializable.
            n_resp, n_ans = reponse_entails_answer.shape
            entailment_details.append(
                {
                    "n_responses": int(n_resp),
                    "n_answers": int(n_ans),
                    "resp_entails_ans": reponse_entails_answer.astype(float).reshape(-1).tolist(),
                    "ans_entails_resp": answer_entails_response.astype(float).reshape(-1).tolist(),
                }
            )

    if return_entailment:
        return ig_scores, entailment_details
    return ig_scores
...
def calculate_uncertainty_hard_batch(
    ...
    for i in range(len(responses)):
        for j in range(i + 1, len(responses)):
            left_texts.append(responses[i])
            right_texts.append(responses[j])

    pairwise_equivalence = []
    for i in range(0, len(left_texts), computation_chunk_size):
        chunk_left = left_texts[i:i + computation_chunk_size]
        chunk_right = right_texts[i:i + computation_chunk_size]
        equivalence_results, _ = check_entail(
            chunk_left,
            chunk_right,
            entailment_model,
            strict_type,
        ...
        pairwise_equivalence.extend(equivalence_results.detach().cpu().numpy().tolist())

    semantic_set_ids, next_id = _connected_component_ids(len(responses), pairwise_equivalence)
    ...
    cluster_matches = np.zeros(n_cluster, dtype=bool)
    cluster_ids, cluster_texts, answer_texts = [], [], []
    for cluster_id, cluster_responses in enumerate(responses_per_cluster):
        for response in cluster_responses:
            for answer in gt_answers:
                cluster_ids.append(cluster_id)
                cluster_texts.append(response)
                answer_texts.append(f"{question} {answer}")

    for i in range(0, len(cluster_texts), computation_chunk_size):
        chunk_cluster_texts = cluster_texts[i:i + computation_chunk_size]
        chunk_answer_texts = answer_texts[i:i + computation_chunk_size]
        ans_equivalence_results, _ = check_entail(
            chunk_cluster_texts,
            chunk_answer_texts,
            entailment_model,
            strict_type,
        ...
        for offset, equivalent in enumerate(ans_equivalence_results.detach().cpu().numpy().tolist()):
            if equivalent:
                cluster_matches[cluster_ids[i + offset]] = True

    ig_hard = 0.
    for cluster_id in range(n_cluster):
        if cluster_matches[cluster_id]:
            ig_hard += np.exp(log_likelihood_per_semantic_id[cluster_id])
...
def process_item_for_ig(item, subsample=-1, n_repeat=0):
    if subsample > 0:
